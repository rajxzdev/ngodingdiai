...
Web app generator berbasis **pipeline AI 9 role** — dari *Web & Reference Analyst* (analisis website referensi) sampai *Design Engineering* —
...
- 🧠 **9 Role AI berurutan** — hasil role sebelumnya menjadi konteks role berikutnya.
- 🌐 **Input URL website** — pilih sebagai *contoh desain* / *contoh sistem*; Web & Reference Analyst menganalisis isinya lalu meneruskannya ke Prompt Engineering.
...
- 🔎 **Search engine model** — **146+ model dari 8 provider** (database JSON, update Agustus 2026), otomatis **terbaru → terlama** (fuzzy match, contoh: `gpt-4.0` menemukan `gpt-4o`), dengan **custom model** sebagai fallback jika tidak ditemukan.
...
## Alur Pipeline (9 Role)

```
Project Brief (+ URL referensi)
...
app/
  ...
  generator/page.tsx      # Generator: brief + URL web → pipeline live (CLIENT-SIDE streaming) → output tabs
...
components/
  ...
  ...
lib/
  roles.ts                # Definisi 9 role + system prompt + konteks rantai
  api/fetch-url/route.ts  # Proxy ambil isi website referensi
...
**Apache License 2.0** — boleh dipakai, dimodifikasi, dan didistribusikan (termasuk komersial) dengan tetap mencantumkan lisensi ini.

📝 Riwayat perubahan lengkap: [CHANGELOG.md](./CHANGELOG.md)
